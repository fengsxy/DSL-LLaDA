...
class DITS(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(self, cfg, vocab_size):
        ...
        # causal toggle—default to False if you don't have it in cfg
        self.causal = bool(getattr(bb, "causal", False))
        self.adaLN = not self.causal
        self.dit_cond = cfg.backbone.dit_cond
        ...
        # remove vocab_embed (see next change)
        if not self.causal:
        ...
        for _ in range(n_blocks):
            blocks.append(
                DDiTBlockCausal(dim, n_heads, dropout=dropout) if self.causal else
                DDiTBlock(dim, n_heads, cond_dim=cond_dim, adaLN=self.adaLN, dropout=dropout)
            )
    ...
    def forward(self, x, noise_mags):
        # x: (B, T, dim_h) already in backbone hidden dim
        # Sahoo says they use a mixture of time and no time conditioning - pretrained should work without!
        if self.dit_cond:
            t_input = noise_mags.mean(dim=-1)
        else:
            t_input = torch.zeros_like(noise_mags.mean(dim=-1))

        sigma = F.silu(self.sigma_map(t_input))  # (B, cond_dim)
        ...
        with torch.cuda.amp.autocast(dtype=torch.bfloat16):
            for blk in self.blocks:
                if self.adaLN:
                    x = blk(x, rotary_cos_sin, c=sigma)
                else:
                    x = blk(x, rotary_cos_sin)

        # keep logits in fp32 for CE stability
        with torch.autocast(device_type='cuda', enabled=False):
            x = self.output_layer(x, c=sigma if self.adaLN else None)
        return x