...
import utils
from vision_datasets.discretized_cifar10 import DummyVisionTokenizer, DiscreteCIFAR10
from vision_datasets.discretized_imagenet import ImageNetVisionTokenizer, ImageNet
...
def get_lambada_test_dataset(cache_dir):
    ...
    meta_data = os.path.join(raw_cache_dir, "lambada_test.jsonl")
    if not utils.fsspec_exists(meta_data):
...
def get_dataset(
    ...
    if dataset_name == "wikitext103":
        ...
        dataset = get_lambada_test_dataset(cache_dir)
...
def get_tokenizer(config):
    if config.data.tokenizer_name_or_path == "text8":
        ...
        tokenizer = DummyVisionTokenizer(
            256,
            32,
            add_mask_token=config.data.add_mask_token,
            add_special_tokens=config.data.add_special_tokens,
        )
    elif config.data.tokenizer_name_or_path == "imagenet_1024":
        tokenizer = ImageNetVisionTokenizer(
            1024,
            checkpoint=config.data.tokenizer_checkpoint,
            add_mask_token=config.data.add_mask_token,
            add_special_tokens=config.data.add_special_tokens,
        )
    elif config.data.tokenizer_name_or_path == "imagenet_16384":
        tokenizer = ImageNetVisionTokenizer(
            16384,
            checkpoint=config.data.tokenizer_checkpoint,
            add_mask_token=config.data.add_mask_token,
...
def get_dataloaders(
    config, tokenizer, skip_train=False, skip_valid=False, valid_seed=None, num_gpus=-1
):
    num_gpus = torch.cuda.device_count() if num_gpus < 0 else num_gpus
    ...
    if skip_train:
        ...
        if "cifar10" in config.data.train:
            train_set = DiscreteCIFAR10(config.data.train, train=True, download=True)
        elif "imagenet" in config.data.tokenizer_name_or_path:
            train_set = ImageNet(config.data.train, train=True)
        else:
    ...
    if skip_valid:
        ...
        if "cifar10" in config.data.valid:
            valid_set = DiscreteCIFAR10(config.data.valid, train=False, download=True)
        elif "imagenet" in config.data.tokenizer_name_or_path:
            valid_set = ImageNet(config.data.valid, train=False)
        else:
            valid_set = get_dataset(
                ...
                streaming=config.data.streaming,
                # num_proc=config.loader.num_workers,
                num_proc=1,  # fix in 20251005, this affects the wrap result. e.g. when in 192, the ptb would have 0 items.
