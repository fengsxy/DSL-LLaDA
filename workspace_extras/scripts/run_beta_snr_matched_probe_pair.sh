...
PAIR_ID=${PAIR_ID:-beta_snr_matched_pair_$(date +%Y%m%d_%H%M)}
...
run_train() {
  ...
  echo "[pair:${PAIR_ID}] train ${run_id} beta=${beta} mu=${mu} sigma=${sigma} snr_max_ln=${snr_max_ln}"
  RUN_ID="${run_id}" \
  STEPS=1000 \
  BETA_INIT="${beta}" \
  SNR_MU="${mu}" \
...
run_eval_one() {
  ...
  echo "[pair:${PAIR_ID}] eval gpu=${gpu} key=${model_key} label=${label} beta_infer=${beta_infer} snr=[${snr_min},${snr_max}] gen=${gen_len}"
...
run_train "${run_a}" "0.5" "3.22" "0.6" "80"
