...
LOG_DIR="${LOG_DIR:-logs/discrete_tuning_phase1_${MODEL}_nfe${NFE}_${STAMP}}"
mkdir -p "$LOG_DIR"
...
for block in "${BLOCK_ARR[@]}"; do
  for eos in "${EOS_ARR[@]}"; do
    ...
    suffix="b${block}_${eos_tag}"

    add_job "open_${suffix}" \
      "$PYTHON -u dsl_llada/eval_sde_gen_formal.py --model_key $MODEL --method remask --nfe $NFE --gen_length 256 --tag tune_open --prompts eval_data/sde_prompts_200.json --max_prompts 100 --block_length $block ${eos_args[*]}"
