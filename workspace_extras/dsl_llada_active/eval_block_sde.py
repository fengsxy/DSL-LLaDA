...
Usage:
    CUDA_VISIBLE_DEVICES=4 python dsl_llada/eval_block_sde.py --gpu 0
...
def load_ffout_lora_if_present(model, ckpt_dir, device):
    """Restore ff_out LoRA tensors saved by the DSL trainer."""
    ...
    for fname in sorted(os.listdir(ckpt_dir)):
        if not fname.endswith(".safetensors"):
            continue
        st = load_file(os.path.join(ckpt_dir, fname), device=str(device))
        for key in wanted:
            if key in st:
                tensors[key] = st[key]
        del st
...
def extract_boxed(text):
    """Extract the last \\boxed{...} answer, allowing one level of nested braces."""
    positions = [m.end() for m in re.finditer(r"\\boxed\{", text)]
...
def format_reasoning_prompt(item, dataset):
    if dataset == "gsm8k":
        return (
...
def get_gold_answer(item):
    return str(item.get("gold_answer", item.get("gold", "")))
...
def run_reasoning_eval(generate_fn, data, dataset="gsm8k", desc="eval"):
    """Run reasoning eval with a generate function: prompt_text -> generated_text."""
    correct = 0
    cases = []
    for item in tqdm(data, desc=desc):
        prompt_text = format_reasoning_prompt(item, dataset)
        try:
            gen_text = generate_fn(prompt_text)
        except Exception as e:
            print(f"  Error on item {item['id']}: {e}")
            cases.append({"id": item["id"], "gold": get_gold_answer(item),
                          "pred": "", "correct": False, "text": f"ERROR: {e}"})
            continue

        pred = extract_prediction(gen_text, dataset)
        gold = get_gold_answer(item)
        is_correct = normalize_answer(pred) == normalize_answer(gold)
        correct += int(is_correct)
        cases.append({
            "id": item["id"],
            "original_idx": item.get("original_idx", item.get("unique_id", -1)),
            "gold": gold,
            "pred": pred,
            "correct": is_correct,
            "text": gen_text[:500],
...
def load_eval_data(args):
    if args.data_file:
        ...
        if not os.path.isabs(data_path):
            data_path = os.path.join(_root, data_path)
    else:
        filename = "gsm8k_100.json" if args.dataset == "gsm8k" else "math_500.json"
        data_path = os.path.join(_root, "eval_data", filename)
    with open(data_path) as f:
        all_data = json.load(f)

    start = max(0, args.start_index)
    end = args.end_index if args.end_index is not None else len(all_data)
    data = all_data[start:end]
    if args.n_problems and args.n_problems > 0:
        data = data[:args.n_problems]
    return data, data_path, start, start + len(data)
...
def main():
    ...
    p.add_argument("--data_file", default=None,
                   help="Optional JSON data file. Defaults to eval_data/gsm8k_100.json or math_500.json.")
    p.add_argument("--start_index", type=int, default=0,
                   help="Start offset into the loaded data file for sharded eval.")
    p.add_argument("--end_index", type=int, default=None,
                   help="End offset into the loaded data file for sharded eval.")
    p.add_argument("--output", default=None,
                   help="Output JSON path. Defaults to eval_results/block_sde.json.")
    ...
    load_ffout_lora_if_present(model, ckpt_path, device)
    ...
    # Load reasoning data
    data, data_path, data_start, data_end = load_eval_data(args)
    print(f"Evaluating on {len(data)} {args.dataset} problems from {data_path} "
    ...
    if not args.skip_baselines and args.only is None:
        ...
        print("=" * 60)
        t0 = time.time()
        acc, correct, cases = run_reasoning_eval(
            ...
            data, dataset=args.dataset, desc="Pure SDE 32"
        )
        ...
        acc, correct, cases = run_reasoning_eval(
            ...
            data, dataset=args.dataset, desc="Remask 64 b256"
        )
        elapsed = time.time() - t0
        ...
        acc, correct, cases = run_reasoning_eval(
            ...
            data, dataset=args.dataset, desc="Remask 64 b32"
        )
        elapsed = time.time() - t0
        ...
        acc, correct, cases = run_reasoning_eval(
            ...
            data, dataset=args.dataset, desc=f"Remask {steps} b{bl}"
        )
        elapsed = time.time() - t0
        results[args.only] = {
            "accuracy": round(acc, 4), "correct": correct, "n": len(data),
        ...
        acc, correct, cases = run_reasoning_eval(
            ...
            data, dataset=args.dataset, desc=f"Pure SDE {steps}"
        )
        elapsed = time.time() - t0
        results[args.only] = {
            "accuracy": round(acc, 4), "correct": correct, "n": len(data),
    ...
    for bs in block_sizes:
        ...
        for spb in steps_list:
            ...
            acc, correct, cases = run_reasoning_eval(
                ...
                data, dataset=args.dataset, desc=f"Block B={bs} S={spb}"
            )
            elapsed = time.time() - t0
            results[config_name] = {
                "accuracy": round(acc, 4), "correct": correct, "n": len(data),
    ...
    out_path = args.output or os.path.join(_root, "eval_results", "block_sde.json")
