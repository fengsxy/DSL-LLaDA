...
files = sorted(glob.glob('/home/ubuntu/efs/RMDM/eval_results/summarization/${DS}_llama31_8b_ar_shard*of${NSHARD}.json'))
samples = []; seen = set()
for f in files:
    d = json.load(open(f))
    for s in d['samples']:
...
json.dump(out, open('/home/ubuntu/efs/RMDM/eval_results/summarization/${DS}_llama31_8b_ar.json', 'w'), ensure_ascii=False, indent=2)
...
for ds in ['xsum','cnn_dailymail','pubmed','arxiv']:
    f=f'/home/ubuntu/efs/RMDM/eval_results/summarization/{ds}_llama31_8b_ar.json'
    try:
        d=json.load(open(f))
