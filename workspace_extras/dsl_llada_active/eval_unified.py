...
Reads model config from eval_results/registry.json, loads model once,
runs 4 test suites (T1 corruption, T2 generation, T3 mask prediction,
T4 reasoning), saves structured JSON results.

Usage:
    python dsl_llada/eval_unified.py --model_key sm_b2 --gpu 0
    python dsl_llada/eval_unified.py --model_key sm_b2 --tests t1,t4 --gpu 0
    python dsl_llada/eval_unified.py --model_key sm_b2 --tests t2 --gpu 0 --sde_beta_infer 1.5
    python dsl_llada/eval_unified.py --model_key original --gpu 0 --skip_existing
    python dsl_llada/eval_unified.py --aggregate
"""
import argparse
import datetime
import glob
...
REGISTRY_PATH = os.path.join(EVAL_RESULTS_DIR, "registry.json")
...
def load_model(checkpoint, device):
    ...
    load_ffout_lora_if_present(model, checkpoint, device)
    return model


def load_ffout_lora_if_present(model, checkpoint, device):
    """Restore ff_out LoRA adapters saved by DSL training, when present."""
    ...
    for fname in sorted(os.listdir(checkpoint)):
        if not fname.endswith(".safetensors"):
            continue
        st = load_file(os.path.join(checkpoint, fname), device=str(device))
        for key in wanted:
            if key in st:
                tensors[key] = st[key]
        del st
    ...
    print(f"  Loaded ff_out LoRA: r={r}, scale={lora.scale}")
    return True
...
def _load_json(name):
    if os.path.isabs(name):
        ...
        path = os.path.join(EVAL_DATA_DIR, name)
    with open(path) as f:
        return json.load(f)


def _result_suffix_for_texts(texts_file):
    stem = os.path.splitext(os.path.basename(texts_file))[0]
    return "" if stem == "texts_100" else f"_{stem}"
...
def run_t1(model, tokenizer, device, model_key, texts_file="texts_100.json", modes=None):
    """Run T1: token corruption tests."""
    print("\n" + "=" * 60)
    print("T1: Token Corruption")
    print("=" * 60)

    modes = set(modes or ["random", "semantic_auto", "semantic_manual"])
    texts_data = _load_json(texts_file)
    result = {
        "test": "t1_corruption",
        "model": model_key,
    ...
    save_result(model_key, f"t1_corruption{_result_suffix_for_texts(texts_file)}.json", result)
    return result
...
def compute_calibration_bins(confidences, accuracies, n_bins=15):
    """Expected Calibration Error plus reliability-bin details."""
...
def compute_ece(confidences, accuracies, n_bins=15):
    ...
    ece, _ = compute_calibration_bins(confidences, accuracies, n_bins=n_bins)
    return ece


def run_t3(model, tokenizer, device, model_key, texts_file="texts_100.json"):
    ...
    for rate_pct in [30, 50, 70]:
        ...
        ece, bins = compute_calibration_bins(all_conf, all_acc)
    ...
    save_result(model_key, f"t3_mask{_result_suffix_for_texts(texts_file)}.json", result)
    return result
...
def main():
    ...
    parser.add_argument("--texts_file", default="texts_100.json",
                        help="Text eval data for T1/T3, relative to eval_data/ or a path")
    parser.add_argument("--t1_modes", default="random,semantic_auto,semantic_manual",
                        help="Comma-separated T1 subtests: random,semantic_auto,semantic_manual")
    ...
    for t in run_order:
        ...
        try:
            if t == "t1":
                t1_modes = [m.strip() for m in args.t1_modes.split(",") if m.strip()]
                run_t1(model, tokenizer, device, args.model_key,
                       texts_file=args.texts_file, modes=t1_modes)
            elif t == "t3":
