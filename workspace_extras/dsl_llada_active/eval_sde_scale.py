...
def main():
    ...
    p.add_argument('--beta_infer', type=float, default=None)
    ...
    reg = json.load(open(os.path.join(_root, 'eval_results/registry.json')))
    ...
    if a.beta_infer is not None:
        bi = a.beta_infer
    ns = a.noise_scale if a.noise_scale is not None else NS_BY_NFE.get(a.nfe, 0.01)
    ...
    outpath = os.path.join(_root, f'eval_results/scale_{a.model_key}_{a.method}_nfe{a.nfe}{suffix}.json')
