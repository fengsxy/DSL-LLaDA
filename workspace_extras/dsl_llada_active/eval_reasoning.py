...
Usage:
    python dsl_llada/eval_reasoning.py \
...
_xdm_spec = importlib.util.spec_from_file_location(
    "generate_xdm_module",
    os.path.join(os.path.dirname(__file__), '..', 'LLaDA-XDLM', 'generate.py'))
...
def load_gsm8k_json(path):
    """Load prepared GSM8K JSON files from eval_data/."""
    with open(path, "r", encoding="utf-8") as f:
        rows = json.load(f)
    problems = []
    for item in rows:
        gold = item.get("gold_answer", item.get("gold", ""))
        problems.append({
            "question": item["question"],
...
def generate_answer(model, tokenizer, question, method, steps, device,
                    ...
                    save_trajectory=False, prompt_style='chat',
                    add_special_tokens=None, **kwargs):
    """Generate answer for a single question. Returns (text, trajectory_or_None)."""
    if block_length is None:
        block_length = gen_length

    if dataset_type == 'gsm8k' and prompt_style == 'official_gsm8k_fewshot':
        prompt_text = GSM8K_OFFICIAL_FEWSHOT.format(question=question)
    ...
    if add_special_tokens is None:
        add_special_tokens = prompt_style != 'chat'
    encoded = tokenizer(
        formatted, add_special_tokens=add_special_tokens, return_tensors="pt")
...
def evaluate(model, tokenizer, problems, method, steps, device,
             ...
             start_index=0,
             gen_length=512, block_length=None, save_trajectory=False,
             prompt_style='chat', add_special_tokens=None, **kwargs):
    ...
    for i, prob in enumerate(tqdm(problems, desc=f'{dataset_type} {method}-{steps}step')):
        try:
            ...
            output, traj = generate_answer(
                ...
                prompt_style=prompt_style, add_special_tokens=add_special_tokens,
                **extra_kwargs)
            ...
            results.append({
                'idx': start_index + i, 'question': prob['question'][:200],
...
def main():
    ...
    parser.add_argument('--data_file', default=None,
                        help='Optional prepared JSON file for GSM8K/other eval data')
    parser.add_argument('--prompt_style',
                        choices=['chat', 'plain', 'official_gsm8k_fewshot'],
                        default='chat')
    parser.add_argument('--no_add_special_tokens', action='store_true',
                        help='Disable tokenizer special tokens for non-chat prompts')
    ...
    parser.add_argument('--start_index', type=int, default=0,
                        help='Start offset into the loaded problem list')
    ...
    tokenizer_path = args.tokenizer_checkpoint or args.checkpoint
    ...
    if args.data_file:
        problems = load_gsm8k_json(args.data_file)
    ...
    accuracy, results, trajectories = evaluate(
        ...
        add_special_tokens=False if args.no_add_special_tokens else None,
    ...
    summary = {
        ...
        'prompt_style': args.prompt_style,
        'add_special_tokens': False if args.no_add_special_tokens else (args.prompt_style != 'chat'),
        'tokenizer_checkpoint': tokenizer_path,
        'data_file': args.data_file,
        'k1': args.k1 if args.method == 'xdm' else None,
        'accuracy': accuracy,
