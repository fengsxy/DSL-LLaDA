...
#
# Usage:
#   bash dsl_llada/eval_sum_full_sweep.sh            # all configs
#   bash dsl_llada/eval_sum_full_sweep.sh xsum       # only xsum
#   SKIP_SDE=1 bash dsl_llada/eval_sum_full_sweep.sh # skip SDE configs (for resume)
set -euo pipefail
...
LAUNCHER="bash dsl_llada/eval_sum_shard_launch.sh"
...
is_done() {
  ...
  /home/ubuntu/efs/RMDM/.venv/bin/python - "$f" <<'PY' 2>/dev/null
import json, sys
...
run_config() {
  ...
  /home/ubuntu/efs/RMDM/.venv/bin/python dsl_llada/merge_summarization_shards.py \
