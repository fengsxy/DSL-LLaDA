...
Usage:
    python dsl_llada/eval/prepare_eval_data.py
"""
...
def prepare_texts_100():
    """Take first 100 texts from results/paper/eval_texts.json, tokenize with LLaDA."""
    from transformers import AutoTokenizer

    src = os.path.join(ROOT, "results", "paper", "eval_texts.json")
    with open(src) as f:
    ...
    out_path = os.path.join(OUT_DIR, "texts_100.json")
    with open(out_path, "w") as f:
        json.dump(records, f, indent=2, ensure_ascii=False)
    print(f"texts_100.json: {len(records)} entries written to {out_path}")
    return records
