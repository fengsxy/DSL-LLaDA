...
def load_model_and_components(model_key, device):
    ...
    registry = json.load(open(os.environ.get("DSL_LLADA_REGISTRY", os.path.join(_root, "dsl_llada", "configs", "registry.json"))))
...
def main():
    ...
    parser.add_argument("--method", choices=["remask", "sde"], default="remask")
    ...
    sde_sch = _sde_schedule(args.model_key)
    ...
    for seed in seeds:
        ...
        out_name = f"{args.tag}_{args.model_key}_{args.method}{eos_tag}{blk_tag}{sup_tag}_nfe{args.nfe}_gen{args.gen_length}{seed_tag}{shard_tag}.json"
        ...
        result = {
            ...
            "sde_params": {
                "beta_infer": sde_comp["beta_train"] * 2.0 if sde_comp else None,
                "noise_scale": _adaptive_ns(args.nfe) if args.method == "sde" else None,
