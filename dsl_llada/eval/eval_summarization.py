...
Usage:
    python dsl_llada/eval/eval_summarization.py \
...
def generate_remask_text(model, tokenizer, text, device, nfe=64,
    ...
    with torch.no_grad():
        out = generate(
            ...
            temperature=0.0, cfg_scale=0.0, remasking=remasking,
...
def main():
    ...
    parser.add_argument(
        "--method",
        required=True,
        choices=["sde", "remask", "rmdm", "rmdm_conf", "rmdm_cap"],
        help="Generation method. rmdm is an alias for confidence-based ReMDM.",
    )
