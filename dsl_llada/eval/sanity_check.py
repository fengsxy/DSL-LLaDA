# dsl_llada/eval/sanity_check.py
...
from dsl_llada.core.dsl_modules import DSLLaDA, sample_mixed_snr, noisy_embedding
...
def test_forward_shape():
    import dsl_llada.core.dsl_modules as m
...
def test_two_stage_generate_logic():
    ...
    from dsl_llada.core.two_stage_generate import two_stage_generate, MASK_ID as _MASK

    dsl = DSLLaDA(MockLLaDA())
    prompt = torch.randint(1, VOCAB, (1, 4))  # 4 prompt tokens (not mask=0)

    # Patch module-level MASK_ID for the tiny test
    import dsl_llada.core.two_stage_generate as tsg
...
def test_llada_cpt_dsl_syntax():
    ...
    import py_compile
    filepath = os.path.join(os.path.dirname(__file__), '..', 'train', 'llada_cpt_dsl.py')
    try:
        py_compile.compile(filepath, doraise=True)
    except py_compile.PyCompileError as e:
