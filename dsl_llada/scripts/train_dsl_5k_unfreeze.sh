#!/bin/bash
# dsl_llada/scripts/train_dsl_5k_unfreeze.sh
# 5K step training with ff_out UNFROZEN (via separate fp32 optimizer)
# Based on train_dsl_5k.sh + separate optimizer fix
# Expected: ~12-15 hours on 8xA100
# Usage: bash dsl_llada/scripts/train_dsl_5k_unfreeze.sh [output_dir]
...
export DSL_FREEZE_FFOUT=0   # KEY: train ff_out via separate fp32 optimizer

OUTPUT_DIR=${1:-"./checkpoints/dsl_5k_unfreeze_$(date +%Y%m%d_%H%M%S)"}

echo "=== DSL 5K: ff_out UNFROZEN (separate fp32 optimizer) ==="
